...
let currentCoord;
function retry() {
    const coord = [Levels.levels[0].cities[currentCoord].position.q, Levels.levels[0].cities[currentCoord].position.r];
    console.log(coord);
...
function map1() {
    currentCoord = 0;
    for (let i = 0; i < 3; i++) {
        const coord = [Levels.levels[0].cities[i].position.q, Levels.levels[0].cities[i].position.r];
        console.log(coord);
        Game.AdjustParams(coord);
        StartGame();
    }
...
function map2() {
    currentCoord = 1;
    for (let i = 0; i < 3; i++) {
        const coord = [Levels.levels[1].cities[i].position.q, Levels.levels[1].cities[i].position.r];
...
function map3() {
    currentCoord = 2;
    for (let i = 0; i < 3; i++) {
        const coord = [Levels.levels[2].cities[i].position.q, Levels.levels[2].cities[i].position.r];
        console.log(coord);
        Game.AdjustParams(coord);
        StartGame();
    }
