...
const numbersHolder = document.getElementById("numbers") as HTMLElement;
const numbersListHolder = document.getElementById("numberslist") as HTMLElement;
const startButton = document.getElementById("startBtn")?.addEventListener('click', Generate);
const lottoMap = document.getElementById("lottomap") as HTMLDivElement;
...
function Generate() {
    ...
    let many: number = Number(amount?.value)
...
function Game() {
    for (let i = 1; i < Number(amount?.value) + 1; i++) {
        let actualNumber = document.querySelector(`.tip${i.toString()}`) as HTMLInputElement;
